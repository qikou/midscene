  ...
  readonly inputPrimitives: ComputerInputPrimitives = {
    pointer: {
      ...
      doubleClick: async ({ x, y }) => {
        assert(libnut, 'libnut not initialized');
        const target = this.toGlobalPoint({ x, y });
        libnut.moveMouse(Math.round(target.x), Math.round(target.y));
      ...
      dragAndDrop: async (from, to) => {
        ...
        libnut.moveMouse(Math.round(globalFrom.x), Math.round(globalFrom.y));
        libnut.mouseToggle('down', 'left');
        await sleep(100);
        libnut.moveMouse(Math.round(globalTo.x), Math.round(globalTo.y));
        await sleep(100);
        libnut.mouseToggle('up', 'left');
      },
    },
