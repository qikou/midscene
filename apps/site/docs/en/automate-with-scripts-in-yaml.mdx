...
Execute ADB shell commands on Android devices. To set a command timeout, keep `timeout` as a shallow sibling of `runAdbShell`.
...
Image URLs can be local paths, Base64 data URLs, or remote links. Base64 data URLs must include the `data:image/...;base64,` prefix. When using image links that cannot be accessed by the model, set `convertHttpImage2Base64: true` so Midscene will download the image and send it as a Base64 data URL.
...
For auto-planning steps like `aiAct`, `aiAction`, and `ai`, put the text and
images in the `instruction` field as a sibling of the action key.

```yaml
tasks:
  - name: Click target logo
    flow:
      - aiAct:
        instruction:
