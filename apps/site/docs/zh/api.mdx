...
function ai(prompt: string | Object): Promise<string | undefined>; // 简写形式
```
...
- 返回值：
  ...
  - 返回一个 Promise。当指令要求 Midscene 返回或告诉你某些信息时，Promise 会解析为最终输出字符串。如果任务只执行操作、没有产出返回信息，则解析为 `undefined`。
  - 若执行失败，则抛出错误。
...
// 让 aiAct 在完成任务后返回信息
const heading = await agent.aiAct(
  '在 Google 搜索 "Midscene"，并且只返回第一个搜索结果页面的标题',
);
expect(heading).toContain('Midscene');
