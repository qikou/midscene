...
如果需要通过命令行执行 YAML 工作流，请查看 [YAML 脚本运行器](./yaml-script-runner)，了解安装、`.env` 支持以及 `midscene` 命令的用法。
...
在 Android 设备上执行 ADB shell 命令。如需设置命令超时，请将 `timeout` 作为 `runAdbShell` 的同级字段。
...
图片 URL 可以是本地路径、Base64 data URL 或远程链接。Base64 data URL 必须包含 `data:image/...;base64,` 前缀。如果图片链接无法被模型访问，请设置 `convertHttpImage2Base64: true`，Midscene 会下载图像并以 Base64 data URL 的形式发送给模型。
...
对于自动规划类步骤，例如 `aiAct`、`aiAction` 和 `ai`，请把文本和图片写在
操作指令同级的 `instruction` 字段下。
